...
import com.tradeagent.domain.client.StockDataClient;
...
public class StockService {
    ...
    /** Create the stock service with data-source, advice, and cache ports. */
    public StockService(StockDataClient stockDataClient, AdviceService adviceService, StockCache stockCache) {
    ...
    public StockIdentity resolveStock(String query) {
        ...
        if (StockText.isSymbolQuery(cleanQuery)) {
            ...
            try {
                StockIdentity resolved = stockDataClient.resolveStock(cleanQuery);
    ...
    public KlinePayload refreshSymbol(String symbol) {
        ...
        synchronized (lock) {
            ...
            try {
                KlinePayload payload = stockDataClient.fetchKline(normalized, name);
                if (payload == null) {
